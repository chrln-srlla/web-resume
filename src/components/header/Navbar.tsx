...
export default function Navbar() {
  ...
  const base =
    "px-3 py-2 cursor-pointer border rounded-full text-xs font-medium transition-colors";
  const activeStyle = "bg-white text-black border-white";
  const inactiveStyle =
    "border-transparent text-white/80 hover:bg-white/10 hover:text-white hover:border-white/10";
    ...
    <nav className="print:hidden relative flex items-center justify-between py-5">
      ...
      >
        <div className="flex items-center border border-white/10 bg-white/5 rounded-full p-1 hover:bg-white/10 transition-colors">
          <div className="text-white text-xs px-4 py-2 font-semibold tracking-wide">
            about
          </div>
